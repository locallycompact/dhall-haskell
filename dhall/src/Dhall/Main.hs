  ...
  where
    ...
    parseResolveMode =
      Options.Applicative.flag'
        ...
        <|> Options.Applicative.flag'
          (Just ListTransitiveDependencies)
  ...
  handle $ case mode of
    ...
    Encode {..} -> do
      ...
      if json
        ...
        else Data.ByteString.Lazy.putStr bytes
    Decode {..} -> do
      bytes <-
        case file of
      ...
      expression <-
        if json
          ...
          else Dhall.Core.throws (Dhall.Binary.decodeExpression bytes)
    ...
    SyntaxTree {..} -> do
      ...
      if noted
        then Text.Pretty.Simple.pPrintNoColor expression
        else
